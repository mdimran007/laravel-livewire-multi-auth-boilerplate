...
class Report extends Component
{
    ...
    {
        $data = ModelsReport::with(['goals', 'creator'])->findOrFail($id);
        $this->dataId = $data->id;
    ...
    {
        $dataList = ModelsReport::with('goals')
            ->orderBy('created_at', 'desc')
            ->paginate(16);

        return view('livewire.admin.asset.report.report', [
            'dataList' => $dataList
        ]);
    }
    ...
    {
        $data = ModelsReport::findOrFail($id);
