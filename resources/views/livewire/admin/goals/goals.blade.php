                                ...
                                @foreach ($goals as $item)
                                     <div wire:key="item-{{ $item->id }}" class="card">
                                            ...
                                            <div class="flex flex-col">
                                                <div class="py-3 px-6">
                                                    <h5 class="my-2"><a href="apps-project-detail.html" class="text-slate-900 dark:text-slate-200">{{ __("Short Description") }}:</a></h5>
                                                    <p class="text-gray-500 text-sm mb-9">{{ $item->short_description }}</p>
                                                    ...
                                                    <div class="grid lg:grid-cols-2 gap-4">
                                                        <div class="flex items-center justify-between gap-2">
                                                            <a href="#" class="text-sm">
                                                                ...
                                                                <span class="align-text-bottom">{{ __("Created") }}: {{ $item->created_at->diffForHumans() }}</span>
                                                            </a>
                                                        </div>
                                                        <div class="flex items-center gap-2">
                                                            <a href="{{ route('admin.goals.details', $item->id) }}" class="btn border-info text-info hover:bg-info hover:text-white">{{ __("Details") }}</a>
                                                            <a href="{{ route('admin.goals.edit', $item->id) }}" class="btn border-primary text-primary hover:bg-primary hover:text-white">{{ __("Edit") }}</a>
                                                            <button type="button" class="btn border-danger text-danger hover:bg-danger hover:text-white" wire:click="$dispatch('confirm-delete', {{ $item->id }})">{{ __("Delete") }}</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
