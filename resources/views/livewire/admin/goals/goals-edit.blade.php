<div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="text-gray-800 text-sm font-medium mb-2 inline-block">Title <span class="text-danger">*</span></label>
                        <input type="text" wire:model="title" class="form-input" placeholder="Title">
                        @error('title') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="text-gray-800 text-sm font-medium mb-2 inline-block">Status <span class="text-danger">*</span></label>
                ...
                <div class="mt-5">
                    <label class="text-gray-800 text-sm font-medium mb-2 inline-block">Short Description <span class="text-danger">*</span></label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                        <label class="text-gray-800 text-sm font-medium mb-2 inline-block">Image</label>
                        <input type="file" wire:model="image" class="form-input p-1.5 border">
                        @if ($old_image)
                            <div class="mt-2">
                                <img src="{{ asset('storage/' . $old_image) }}" alt="Goal Image" class="h-20 w-20 object-cover rounded">
                            </div>
                        @endif
                ...
                <div class="mt-6">
                    <h6>Select max 4 items for showing on the counter.</h6>
                    <div class="flex flex-wrap gap-4 my-3">
                        @foreach (goalItemList() as $key => $item)
                            <div class="flex items-center gap-2">
                                <input type="checkbox" wire:model="selectedItems" value="{{ $item }}" class="form-checkbox rounded border-gray-300">
                                <label class="text-gray-800 text-sm font-medium">{{ Str::ucfirst($item) }}</label>
                            </div>
                        @endforeach
                    </div>
